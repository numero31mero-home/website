...
gsap.to("#svg5", {y: 0, x: 0, scrollTrigger: {
  trigger: "body",
  scrub: true,
  start: () => window.innerHeight * 8 + " bottom",
  end: () => window.innerHeight * 9 + " bottom"
}, ease: "none"})

var svg5TL = gsap.timeline({scrollTrigger: {
  trigger: "body",
  scrub: 2,
  start: () => window.innerHeight * 9 + " bottom",
  end: () => window.innerHeight * 10 + " bottom"
}})

svg5TL.fromTo("#svg5 video, #svg5 img", {y: "30%"}, {y: 0}, 0)
svg5TL.to("#svg__star5", {morphSVG: {shape: "#svg__rec5", origin: "50% 50%"}}, 0);
